...
class SyncN8nTests(unittest.TestCase):
 def base(self):return {"id":"wf","name":"LHOS","active":True,"nodes":[{"name":"Existing","id":"existing","type":"n8n-nodes-base.httpRequest","credentials":{"httpHeaderAuth":{"id":"cred","name":"Header"}}}],"connections":{},"settings":{"executionOrder":"v1"},"versionId":"do-not-send"}
 ...
 def test_update_payload_omits_read_only_and_activation_fields(self):
  payload=update_payload(patch_workflow(self.base()));self.assertEqual(set(payload),{'name','nodes','connections','settings'});self.assertNotIn('active',payload);self.assertNotIn('id',payload)
