...
class IrisDashboardHealthTests(unittest.TestCase):
 ...
 def test_navigation_scrolls_to_page_sections(self):
  ...
  self.assertIn("addEventListener('hashchange'",DASHBOARD_HTML)
 ...
 def test_hosted_watchdog_drift_within_90_minutes_needs_no_observation(self):
  now=datetime(2026,7,26,17,38,tzinfo=ET);watch=(now-timedelta(minutes=34)).isoformat();gate=now.replace(hour=15,minute=0).isoformat()
  s=self.snap(now=now,state={"stage":"not_sent","content_valid":False},heartbeat={"watchdog":watch,"auto_send":gate})
  self.assertEqual(s["systems"]["watchdog"]["light"],"green")
  self.assertEqual(s["overall"]["light"],"green")
  self.assertIn("no action",s["systems"]["watchdog"]["detail"].lower())
 def test_daytime_watchdog_over_90_minutes_is_orange_not_failure(self):
  now=datetime(2026,7,26,11,0,tzinfo=ET);watch=(now-timedelta(minutes=100)).isoformat();fresh=(now-timedelta(seconds=20)).isoformat()
  s=self.snap(now=now,state={"stage":"hold"},heartbeat={"watchdog":watch,"prepare":fresh,"check_replies":fresh})
  self.assertEqual(s["systems"]["watchdog"]["light"],"orange")
  self.assertEqual(s["overall"]["light"],"orange")
  self.assertNotIn("needs observation",s["overall"]["detail"].lower())
  self.assertIn("no confirmed failure",s["overall"]["detail"].lower())
 def test_evening_is_idle_until_next_morning(self):
  now=datetime(2026,7,26,20,0,tzinfo=ET);watch=(now-timedelta(minutes=121)).isoformat();gate=now.replace(hour=15,minute=0).isoformat()
  s=self.snap(now=now,state={"stage":"not_sent","content_valid":False},heartbeat={"watchdog":watch,"auto_send":gate})
  self.assertEqual(s["overall"]["mode"],"idle")
  self.assertEqual(s["overall"]["light"],"green")
  self.assertEqual(s["overall"]["next_start"],datetime(2026,7,27,7,0,tzinfo=ET).isoformat())
 def test_calm_status_language_replaces_fear_inducing_legend_copy(self):
  self.assertNotIn('One signal needs observation',DASHBOARD_HTML)
  self.assertNotIn('Observe or expected input',DASHBOARD_HTML)
  self.assertNotIn('Degraded performance',DASHBOARD_HTML)
  self.assertIn('Waiting or delayed',DASHBOARD_HTML)
  self.assertIn('No confirmed failure',DASHBOARD_HTML)
 def test_daytime_stale_watchdog_remains_red(self):
  now=datetime(2026,7,26,11,0,tzinfo=ET);stale=(now-timedelta(minutes=181)).isoformat();fresh=(now-timedelta(seconds=20)).isoformat()
