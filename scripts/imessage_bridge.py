...
import httpx
ET=ZoneInfo("America/New_York"); CHAT_ID=1079; CHAT_GUID="any;+;2232f1a0c45c4a14832adff0ec8120da"
...
def load():
 ...
 except:return {"processed":[],"sent_hashes":[],"outbound_guids":[],"last_seen_id":0}
def save(s):
...
def verify():
 c=sqlite3.connect(os.path.expanduser('~/Library/Messages/chat.db'));row=c.execute('SELECT display_name,guid FROM chat WHERE ROWID=?',(CHAT_ID,)).fetchone();handles=[r[0] for r in c.execute('SELECT h.id FROM chat_handle_join j JOIN handle h ON h.ROWID=j.handle_id WHERE j.chat_id=?',(CHAT_ID,))]
 if not row or row!=("FFAI",CHAT_GUID) or set(map(hh,handles))!=set(NAMES):raise RuntimeError('FFAI identity or participant allowlist mismatch')
def ndjson(t):
 out=[]
...
def send_group(text,s,dry=False):
 verify();text=signed(text);key=hashlib.sha256(text.encode()).hexdigest()
 if key in s['sent_hashes']:return {'action':'duplicate_suppressed'}
 if dry:return {'action':'would_send','text':text}
 r=subprocess.run(['/opt/homebrew/bin/imsg','send','--chat-id',str(CHAT_ID),'--service','imessage','--no-sms-fallback','--text',text,'--json'],capture_output=True,text=True,timeout=45)
 match=next((x for x in history(40) if x.get('is_from_me') and (x.get('text') or '').strip()==text),None)
 if r.returncode and not match:raise RuntimeError('iMessage send uncertain and not found in FFAI history')
 s['sent_hashes'].append(key)
 if match and match.get('guid'):s['outbound_guids'].append(match['guid'])
 save(s);return {'action':'sent','guid':match.get('guid') if match else None}
def actor(row):
...
def direct(row,s):
 t=(row.get('text') or '').lower();reply=row.get('reply_to_guid') or row.get('associated_message_guid')
 if re.search(r'\b(iris|zilla)\b',t) or (reply and reply in s.get('outbound_guids',[])):return True
 explicit=any(x in t for x in ('send the email','send it','good send it','good to send','approved','approve this','ship it'))
 revise=any(x in t for x in ('change','revise','edit','replace','remove','correct','update')) and any(x in t for x in ('email','draft','review'))
 return explicit or revise
def run(dry=False,initialize=False,health=False):
 ...
 if 7<=now.hour<9:
  ...
  for i,row in enumerate(new):
   guid=row.get('guid') or str(row.get('id'));text=(row.get('text') or '').strip();who=actor(row)
   if not who or not text or guid in s['processed']:continue
   if direct(row,s):
   ...
   elif any(w in text.lower() for w in WORDS) and not conversation(new,i):out['actions'].append(send_group(f"{who}, were you talking to me about today’s LifeHouse OS email? Please mention Iris or Zilla and state approve, hold, or the exact revision.",s,dry))
   s['processed'].append(guid)
