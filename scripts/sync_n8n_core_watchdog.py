...
def api(base,key,path,method="GET",body=None):
    ...
    except urllib.error.HTTPError as exc:raise RuntimeError(f"n8n API {method} {path} failed with HTTP {exc.code}") from None

def update_payload(workflow):
    return {key:workflow[key] for key in ("name","nodes","connections","settings") if key in workflow}
