...
class SecurityTests(unittest.TestCase):
 ...
 def test_security_policy_is_read_only_and_fail_closed(self):
  r=self.c.get('/api/lhos/security-policy',headers={'x-lhos-automation-token':'auto'});self.assertEqual(r.status_code,200);body=r.json();self.assertFalse(body['machine_token_decision']);self.assertFalse(body['machine_token_late_send']);self.assertFalse(body['machine_token_production_approval']);self.assertEqual(body['production_approval_evidence'],'authenticated_bound_review_email');self.assertTrue(body['iris_creative_fallback']);self.assertTrue(body['iris_creative_requires_review']);self.assertEqual(body['iris_creative_failover'],'validated_curated');self.assertEqual(body['iris_creative_model'],'glm-4.7-flash');self.assertFalse(body['iris_creative_provider_configured'])
